...
def TVDCondat_1d(y, lamda):
	...
	if N <= 1: 
		x = y 
		return x
	x             = np.zeros(len(y),dtype=float)# y can be a row or column vector.
	...
	x_low_first      = float(y[0] - lamda)   #value of the first segment of the part of x_low under construction
	x_up_first       = float(y[0] + lamda)  # same for x_up
		...
		if y[i] >= x_low_curr:
			...
			if j_low == jseg:  # a jump in x upwards is possible  
				while (x_low_curr >= x_up_first) and (jseg < j_up):
					...
					x_low_curr                     = x_low_curr + (x_low_curr - x_up_first)*(float((indstart_up[jseg]-indjseg))/(i-indstart_up[jseg]+1))
			...
			x_up_curr            = x_up_curr + float(y[i] - x_up_curr)/(i- indstart_up[j_up]+1)      
			x[indjseg]           = x_up_first

			while (j_up > jseg) and (x_up_curr <= x[indstart_up[j_up-1]]):
				j_up       = j_up - 1
				x_up_curr  = x[indstart_up[j_up]] + (x_up_curr - x[indstart_up[j_up]])*(float(i-indstart_up[j_up+1]+1)/(i-indstart_up[j_up]+1))
			...
			if j_up == jseg:  # a jump in x downwards is possible 
				while (x_up_curr <= x_low_first) and (jseg < j_low): 
					...
					jseg                             = jseg + 1
					x[indjseg:indstart_low[jseg]]    = x_low_first
					x_up_curr                        = x_up_curr + (x_up_curr - x_low_first)*(float(indstart_low[jseg] - indjseg)/(i - indstart_low[jseg] + 1))
					indjseg                          = indstart_low[jseg]
					x_low_first                      = x[indjseg]
	...
	if y[i] + lamda <= x_low_curr: 
		...
		# fusion of x_low to keep it nonincreasing
		x_low_curr = x_low_curr + float(y[i] + lamda - x_low_curr)/(i - indstart_low[j_low] + 1)      
		x[indjseg] = x_low_first
		while (j_low > jseg) and (x_low_curr >= x[indstart_low[j_low-1]]):
			j_low      = j_low - 1
			x_low_curr = x[indstart_low[j_low]] + (x_low_curr - x[indstart_low[j_low]])*(float(i-indstart_low[j_low+1]+1)/(i-indstart_low[j_low]+1))
		...
		if j_low == jseg: # the segments of x_up must be validated
			if x_up_first >= x_low_curr: # same unique segment of x_low and x_up
				...
				x_up_curr   = x_up_curr + float(y[i] - lamda - x_up_curr)/(i - indstart_up[j_up] + 1)      
				x[indjseg]  = x_up_first
				while (j_up > jseg) and (x_up_curr <= x[indstart_up[j_up-1]]):
					j_up      = j_up - 1
					x_up_curr = x[indstart_up[j_up]] + (x_up_curr - x[indstart_up[j_up]])*(float(i - indstart_up[j_up+1]+1)/(i - indstart_up[j_up] + 1))
